...
class DQNAgent:
    def __init__(self, input_dim, action_dim, learning_rate=1e-3):
        ...
        self.q_network = DQN(input_dim, action_dim).to('cuda')
        self.target_network = DQN(input_dim, action_dim).to('cuda')
...
def train():
    ...
    for episode in range(episodes):
        ...
        state = torch.tensor(game.cells, dtype=torch.float32).unsqueeze(0).to('cuda')
        done = False
        while not done:
            ...
            next_state = torch.tensor(next_state.cells, dtype=torch.float32).unsqueeze(0).to('cuda')
            reward = torch.tensor([reward], dtype=torch.float32).to('cuda')
            done = torch.tensor([done], dtype=torch.uint8).to('cuda')
